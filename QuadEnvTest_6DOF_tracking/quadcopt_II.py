...
class QuadcoptEnv_6DOF(gym.Env):
  ...
  def __init__(self, Random_reset=False, Process_perturbations=False, NumEpi=25010, Lx=0.34, Ly=0.34, motor_mass=0.04, body_mass=0.484,
    ...
    # NumEpi vecchio = 30000 09/03/2022, cambiato per osservazione del numero indicativo di episodi in un allenamento di 10^7
    ...
    # previous comment
    self.Obs_normalization_vector = np.array([1., 1., 1., 1., 1., 1., 1., 1., 1., 20., 20., 20., 10., 10., 10., 10., 10., 10.]) # normalization constants
    # Random funcs
    ...
    self.gamma = np.pi / 100. * np.arange(200, dtype=np.float32)
    self.x_coordinates = [0.]
    self.y_coordinates = [0.]
    for i in range(int(len(self.gamma))):
      self.x_circle = cos(self.gamma[i])
      self.x_coordinates.append(self.x_circle)
      self.y_circle = sin(self.gamma[i])
      self.y_coordinates.append(self.y_circle)
  ...
  def reset(self):
      ...
      Reset state 
      """
      print("Distance from the the path is --->", self.min)
  ...
  def getReward(self, action):
      ...
      self.dist_vec = [5.]
      self.min = self.dist_vec[0]
      for i in range(int(len(self.x_coordinates))):
          x_err = self.x_coordinates[i] - x
          y_err = self.y_coordinates[i] - y
          z_err = 0. - z
          self.dist = np.sqrt(x_err**2 + y_err**2 + z_err**2)
          self.dist_vec.append(self.dist)
      for j in range(len(self.dist_vec)):
          if (self.dist_vec[j] < self.min):
              self.min = self.dist_vec[j]
      ...
      R = 1 - minweight * minnorm - chiweight * chi - actionweight * actionnorm
